...
{
  unsigned int *piVar3 = pakHead->offset;
  int iVar1 = *piVar3;

  for (auto i = pakHead->pack_num; i > 0; i--)
  {
    piVar3 = RelOffsetToPtr<unsigned int>(piVar3, iVar1 + 0x10);
    iVar1 = *piVar3;
  }

  return (void*) PakAlign128((int) piVar3 + 4);
}
...
int Pk2GetAddr(PK2_HEAD *pakHead, int pakId)
{
  ...
  {
    return (int) pakHead->pad + pakHead->offset[pakId] - 4;
  }
...
void* GetFileInPak(PK2_HEAD *pak_file, int pak_id)
{
  ...
  int offset_index = *file_adrs;

  while( true ) {
    file_adrs = (unsigned int *)((int) file_adrs + offset_index + 0x10);
    pak_id -= 1;
